...
func _physics_process(delta: float) -> void:
	...
	right_joy_dir = Vector2(-Input.get_joy_axis(0, 2), -Input.get_joy_axis(0, 3))
	
	if right_joy_dir.length() > 0:
		right_joy_dir = right_joy_dir.normalized()
		
		rotation = right_joy_dir.angle()
	...
	var direction := Input.get_axis("ui_gamepad_Left", "ui_gamepad_Right")
	if direction:
	...
	if _is_on_ground():
		is_boost = false
		checkStatus = 1
		if Input.is_action_just_pressed("ui_gamepad_jump"):
			velocity.y = JUMP_VELOCITY
	elif Input.is_action_pressed("ui_gamepad_jump") and fuel >= 0.0 and checkStatus == 1:
		is_boost = true
		velocity.y = BOOST
		fuel -= delta
		if fuel < 0.001:
			checkStatus = 0
			print(checkStatus)
	elif Input.is_action_pressed("ui_gamepad_jump") and fuel > 0.2 and checkStatus == 0:
		is_boost = true
		velocity.y = BOOST
		fuel -= delta
		checkStatus = 1
	...
	if Input.is_action_just_pressed("ui_gamepad_shoot"):
		shoot()
...
func shoot():
	...
	bullet.bullet_direction = (right_joy_dir)
